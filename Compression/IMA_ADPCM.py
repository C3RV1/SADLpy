...
class ImaAdpcm:
    ...
    def decompress(data: bytearray) -> bytearray:
        ...
        for i in range(0, len(data)):
            ...
            result.extend(BitConverter.get_bytearray(new_sample))
    ...
    def compress_block(data: bytearray, block_size: int) -> list:
        ...
        for i in range(0, len(data), 2):
            if i % block_size == 0:
                ...
                block = bytearray()
                block.extend(BitConverter.get_bytearray(predicted_sample))
                block.extend(BitConverter.get_bytearray(index))
